...
// Главная страница интернет-магазина чая
const HomePage = () => {
  // Хуки для навигации и управления состоянием
  const dispatch = useDispatch();
  const navigate = useNavigate();
  const catalogRef = useRef(null); // Реф для скролла к каталогу
  ...
  const handleAddToCart = (product) => {
    ...
    dispatch(addToCartDB(product))
      .unwrap()
      .then(() => {
        dispatch(syncCart()); // Синхронизация корзины после добавления
      })
      .catch(error => {
